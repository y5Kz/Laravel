    ...
    <div class="row justify-content-center">
        ...
        <table class="table">
            <tbody class="reply">
                @foreach($rep as $reps)
                <tr>
                    <td scope="col" class='text-center'>{{ $reps['user']['name'] }}<br>{{ $reps['reply'] }}<br>{{ $reps['created_at'] }}</td>
                </tr>
                @endforeach
                <tr>
                    <th scope='col'>
        ...
        <form action="{{ route('ajax.button')}}" method="post">
            ...
            <div class="form-group">
                ...
                <div class='row justify-content-around mt-2 mb-5'>
                    <button type="button" class="btn btn-outline-success" id="fire">投稿</button>
                </div>
            </div>
        </form>
    </div>
